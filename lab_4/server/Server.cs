...
namespace server
{
    class Server
    {
        ...
        public Server(int _port)
        {
            ...
            functions["DeleteLine"] = new Func<string[], Status>(controller.DeleteLine);
        ...
        private void RequestCallback(IAsyncResult ar)
        {
            ...
            if(functions.ContainsKey(func_str))
            {
                ...
                else if (data_splited[1].ToLower() == "null")
                {
                    string[] arguments = new string[data_splited.Length-2];
                    for (var i = 2; i < data_splited.Length; i++)
                    {
                        arguments[i-2] = data_splited[i];
                    }
                    outcome = (Status)functions[func_str].DynamicInvoke((object)arguments);
                }
                else
                {
